...
def appmetadata() -> AppMetadata:
    ...
    metadata = AppMetadata(
        ...
        url="https://github.com/clamsproject/app-datimex-extraction",  # a website where the source code and full documentation of the app is hosted
    ...
    try:
        analyzer_version = [
            l.strip().rsplit('==')[-1] for l in open(pathlib.Path(__file__).parent / 'requirements.txt').readlines()
            if re.match(r'^clams-python==', l)
        ][0]
        metadata.analyzer_version = analyzer_version
    except IndexError:
        print("Warning: clams-python version not found in requirements.txt. Using default version 'unknown'.")

    
    # and then add I/O specifications: an app must have at least one input and one output
    metadata.add_input(DocumentTypes.Document)
    metadata.add_output(AnnotationTypes.Annotation, typeSpecificProperty='property-value')
    ...
    metadata.add_parameter(name='input_video', 
